...
for %%I in ("netcoreapp3.1","net461","net481","net6.0","net8.0") DO (
    ...
    del /q "%ARTIFACTS_CREDENTIALPROVIDER_MSAL_FILECACHE_LOCATION%" 2>NUL
    echo Testing %%I with ARTIFACTS_CREDENTIALPROVIDER_MSAL_ALLOW_BROKER=!ARTIFACTS_CREDENTIALPROVIDER_MSAL_ALLOW_BROKER!
    ...
    dotnet run --no-restore --no-build -f %%I --project CredentialProvider.Microsoft\CredentialProvider.Microsoft.csproj -- -C -U !TEST_FEED! -V Debug ^
        > test.%%I.%ARTIFACTS_CREDENTIALPROVIDER_MSAL_ALLOW_BROKER%.log
    IF !ERRORLEVEL! NEQ 0 (
