...
CREDENTIAL_PROVIDER_VERSION_NORMALIZED=$(echo "${AZURE_ARTIFACTS_CREDENTIAL_PROVIDER_VERSION}" | sed 's/^v//')
...
if [ ! -z ${ARTIFACTS_CREDENTIAL_PROVIDER_RID} ]; then
  ...
  case ${CREDENTIAL_PROVIDER_VERSION_NORMALIZED} in 
    0.*|1.0.*|1.1.*|1.2.*|1.3.*)
...
if [ ! -z ${CREDENTIAL_PROVIDER_VERSION_NORMALIZED} ] && [ ${CREDENTIAL_PROVIDER_VERSION_NORMALIZED} != "latest" ]; then
  # browser_download_url from https://api.github.com/repos/Microsoft/artifacts-credprovider/releases/latest
  URI="https://github.com/$REPO/releases/download/v${CREDENTIAL_PROVIDER_VERSION_NORMALIZED}/$FILE"
else
